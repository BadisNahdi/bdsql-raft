...
public class MainCli {
    public static void main(String[] argv) throws Exception {
        ...
        if ("start-node".equals(cmd)) {
            if (argv.length < 3) {
                System.err.println("start-node needs nodeName and host:port");
                return;
            }
            ...
            if (argv.length < 2) {
                System.err.println("shell needs httpHost:port");
                return;
            }
            String http = argv[1];
            runShell(http);
    ...
    private static void runShell(String httpHostPort) throws Exception {
        ...
        System.out.println(
                "Connected to " + httpHostPort + ". Commands: put key value | get key | del key | info | exit");
        while (true) {
            System.out.print("> ");
            ...
            if ("exit".equals(op) || "quit".equals(op))
                break;
